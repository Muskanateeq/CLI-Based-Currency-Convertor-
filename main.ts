...
let apilink =
  "https://v6.exchangerate-api.com/v6/ff9d5a8866723066f0afa3c7/latest/USD";
...
let useramount = await inquirer.prompt({
  type: "input",
  name: "amount",
  message: `Enter amount to convert in ${chalk.yellow.bold(
    currencyConverter2.currency2
  )}:`,
...
// result view
console.log(
  `your ${chalk.greenBright.bold(useramount.amount)} ${chalk.yellow.bold(
    currencyConverter.currency
  )} in ${chalk.yellow.bold(
    currencyConverter2.currency2
  )} is ${chalk.greenBright.bold(convertedamount)}`
);
